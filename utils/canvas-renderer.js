...
const { createCanvas, Image, GlobalFonts, loadImage, Path2D } = require('@napi-rs/canvas');
 ...
 * @param {{ data: object, format: "jpeg"|"png"|"webp" }} opts
 * @returns {Promise<{ buffer: Buffer, mime: string }>}
 */
const renderCard = async ({ data, format = 'jpeg' }) => {
  ...
  const capAscent = bm.actualBoundingBoxAscent ?? bm.fontBoundingBoxAscent ?? 25;
  const capDescent = bm.actualBoundingBoxDescent ?? bm.fontBoundingBoxDescent ?? 5;
  ...
  switch (format) {
    case 'png':
      ...
      break;
    case 'webp':
      buffer = await canvas.encode('webp', 90);
      mime = 'image/webp';
      break;
    default:
